...
for R1_FILE in "$RAW_DIR"/*_R1_001.fastq.gz; do
  ...
  echo "[1/7] Calculating initial stats..."
  ...
  echo "[3.5.7] Cleaning reads (polyA/T trimming, N removal, length filter)..."
  bbduk.sh \
  ...
  # ----- Step 5: sort and index the BAM -----
  echo "[5/7] Sorting and indexing BAM..."
  ...
  # ----- Step 7: Final stats -----
  echo "[7/7] Final cleaned FASTQ read count..."
